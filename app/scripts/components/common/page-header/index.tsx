...
export default function PageHeader(props: PageHeaderProps) {
  ...
  const testItemsMenu = [<>
      <USWDSNavDropDownButton onToggle={(): void => {
      onToggle();
    }} menuId="testDropDownOne" isOpen={isOpen[0]} label="Nav Label" isCurrent={true} />
      <USWDSMenu key="one" items={testMenuItems} isOpen={isOpen[0]} id="testDropDownOne" />
  ...
  return <>
      <USWDSHeader extended={true} showMobileOverlay={expanded}>
        ...
        <USWDSExtendedNav primaryItems={testItemsMenu} secondaryItems={testMenuItems} mobileExpanded={expanded} onToggleMobileNav={onClick}>
        </USWDSExtendedNav>
      </USWDSHeader>
    </>;
}
