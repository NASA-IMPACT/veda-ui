...
const stories = JSON.parse(require('fs').readFileSync('test/playwright/playwrightTestData.json', 'utf8'))['stories'];
  ...
  if (item !== "External Link Test" && item !== "Internal Link Test") {
    test(`${item} routes from stories to details page`, async({
      ...
      await test.step(`click on ${item} article card`, async() => {
        const storyCard = storiesPage.mainContent.getByRole('article').getByRole('heading', { level: 3, name: item, exact: true}).last();
...
test("external link on Stories page opens in new tab", async({
  ...
  await test.step('click on External Link Test card', async() => {
    const storyCard = storiesPage.mainContent.getByRole('article').getByRole('heading', { level: 3, name: "External Link Test", exact: true}).last();
    await storyCard.scrollIntoViewIfNeeded();
    await storyCard.click({force: true});
  })
...
test("internal link on Stories page opens in same tab", async({
  ...
  await test.step('click on Internal Link Test card', async() => {
    const storyCard = storiesPage.mainContent.getByRole('article').getByRole('heading', { level: 3, name: "Internal Link Test", exact: true}).last();
    await storyCard.scrollIntoViewIfNeeded();
    await storyCard.click({force: true});
  })
