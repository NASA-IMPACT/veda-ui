...
test.describe('ensure links in top navigation route to expected page', async () => {
  test('test link', async({page, headerComponent}) => {
    await page.goto('/');
    await expect(headerComponent.navigation, 'header should load').toBeVisible()
    await headerComponent.clickLink('test');
    await expect(headerComponent.testDropdownLink, 'dropdown link should be visible').toBeVisible();
  ...
  test('exploration link', async({page, headerComponent, datasetSelectorComponent}) => {
    ...
    await expect(datasetSelectorComponent.header, 'data selector header should load').toBeVisible();
    await expect(page, 'should route to /exploration').toHaveURL(/\/exploration/i);
  })

  test('stories link', async({page, headerComponent, storiesPage}) => {
    await page.goto('/');
    await expect(headerComponent.navigation, 'header should load').toBeVisible()
    await headerComponent.clickLink('stories');
    await expect(storiesPage.header, 'stories page header should load').toBeVisible();
    await expect(page, 'should route to /stories').toHaveURL(/\/stories/i);
  })
