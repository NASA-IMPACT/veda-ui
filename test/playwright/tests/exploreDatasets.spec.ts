...
const datasetIds = JSON.parse(
  fs.readFileSync('test/playwright/playwrightTestData.json', 'utf8')
).datasetIds;
const disabledDatasets = JSON.parse(
  fs.readFileSync('test/playwright/playwrightTestData.json', 'utf8')
).disabledDatasets;
  ...
  for (const dataset of datasetIds) {
    test(`${dataset} explore page functions`, async ({
      ...
      if (disabledDatasets.includes(dataset)) {
        await expect(
          datasetSelectorComponent.noDatasetMessage,
          'dataset set to disabled'
        ).toBeVisible();
      } else {
        const collectionsResponsePromise =
          test.step('wait for collect api response', () => {
        ...
        const datasetName = await datasetSelectorComponent.article
          .first()
        ...
        await expect(
          explorePage.firstDatasetItem
            .getByRole('heading', { name: datasetName })
            .first(),
          `article with name ${dataset} should be visible`
        ).toBeVisible();
